...
SELECT 
		...
		, INSTR('축구농구야구', '농구')
		, LOCATE('야구', '축구농구야구'); 
-- INSTR이랑 LOCATE는 부분 문자열과 기준 문자열 위치만 서로 반대
...
-- menu_name이랑 menu_price랑 같이 보고 싶을 때
...
-- 끝까지 잘라낼 거면 두 번 써서 넣을 필요 없음
-- 11번째 문자부터 끝까지
SELECT SUBSTRING('열심히 db공부를 해 봅시다!', 11);
...
-- 윤년도 계산해줌
SELECT ADDDATE('2023-02-01', INTERVAL 28 DAY);

SELECT SUBDATE('2020-02-01', INTERVAL 1 DAY);
SELECT SUBDATE('2020-02-01', 1);
...
-- 암시적 형변환이 됨
SELECT DAY('2023-12-28');
-- 명시적 형변환으로 해줘야 함
SELECT DAY(CAST('2023-12-28' AS DATE)); 
...
-- QUARTER(날짜)
SELECT QUARTER(CURDATE()), QUARTER('2023-12-28');
